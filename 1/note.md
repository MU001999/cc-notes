...
丘奇-图灵论文指出，每一种物理上可实现的计算设备，都可以通过图灵机进行模拟。这意味着，在任何其他计算模型上，可计算问题的集合都不会更大。（但是 CT 论文不是定理，仅仅是对我们目前理解的世界本质的一种信念）。*量子计算机暂时除外，尽管目前尚未证明 $BQP\neq P$*。

 ### 1.6.2 P 的哲学意义

P 类问题被认为可以捕获能通过“可行的”（feasible）的判定程序解决的判定问题的概念。但可能有人质疑 DTIME($n^{100}$) 是否表示现实中的“可行”计算。在实践中，每当我们表明一个问题是 P 类问题时，通常可以找到 $n^3$ 或 $n^5$ 时间的算法。有时候一个问题的第一个多项式时间算法可能会有很高的复杂度，比如 $n^{20}$，但很快会有人简化为 $n^5$ 的时间复杂度。

从程序员的角度来看，P 也是一个自然类。假设一个程序员被要求发明“高效”计算的定义，他会同意以线性或二次时间运行的计算是“有效的”。接下来，由于程序员经常编写调用其他程序(或子程序)的程序，如果一个程序只执行“有效”的计算并调用“有效”的子程序，他可能会自然地认为它是“有效的”。最终得到的“有效计算”的概念就是 P 类问题。

注意：P 类只在特定的上下文中有用。例外如一个人需要设计能够在最新的 PC 上在几分之一秒内运行的算法。

### 1.6.3 对 P 的批评及应对措施

- **最坏情况下的精确计算太严格**，P 的定义只考虑最坏情况，而实践中并非所有可能的输入都出现，因此可以使用平均复杂性。
- **其它物理上可实现的模型**
  - *精度问题*：TM 使用离散符号计算，而物理量可能是实数，由于精度问题，TM 只能近似模拟这种计算。但是由于现实中的设备也会受到噪声的影响，物理量只能以有限的精度测量，因此物理计算的过程中并不能涉及任意的精度，所以 TM 可以使用有限的精度来模拟。
  - *随机性的使用*：对于可以扔硬币的图灵机，它对应的复杂类 BPP 和 P 类似。一种可能是随机计算不比确定性计算更强大。
  - *量子力学的应用*：量子计算机目前似乎只能有效地解决极少数未知的物理问题。因此，从研究物理中获得的一些见解仍然适用于量子计算机。
  - *运用其他奇异的物理，比如弦理论*：到目前为止，似乎许多这样的物理理论产生了同一个等级的BQP，尽管还有许多有待理解。
- **决策问题限制太多**，有些计算问题不容易表达为判定问题，如计算非布尔函数、解决搜索问题、近似优化问题、交互等任务。
